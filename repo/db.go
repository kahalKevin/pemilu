...
func NewRepository(db *sqlx.DB) UserRepository {
	...
	r.getPendukungFull = r.MustPrepareStmt("select p.id, p.nik, name, phone, witness, gender, status, provinsi, kabupaten, kecamatan, kelurahan, tps, photo, address from Pendukung p inner join Dukungan d on p.nik=d.nik where p.nik=? and tingkat=?")
